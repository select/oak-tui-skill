 ...
 * Uses TMUX_PANE env var which is set when the pane is created and doesn't change
 */
export function getTmuxPaneId(): string | null {
  // First try TMUX_PANE env var - this is the pane where the process started
  const envPaneId = process.env.TMUX_PANE;
  if (envPaneId != null && envPaneId !== "") {
    return envPaneId;
  }
  
  // Fallback to display-message (but this returns the ACTIVE pane, not necessarily ours)
  try {
